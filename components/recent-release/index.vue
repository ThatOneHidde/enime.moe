<template>
  <arrows>
    ...
    >
      <nuxt-link
        :nuxt-child-key="episode.id"
        :to="`/watch/${episode.anime.slug}/${episode.number}`"
        :key="episode.id"
        v-for="(episode, index) in recent"
        class="p-0 m-0"
      >
        <episode-card
          :key="episode.id"
...
const { data } = await useFetch(
  () =>
    `${runtimeConfig.public.enimeApi}/recent?page=${page.value}&perPage=${rows}`,
  {
...
function load() {
  if (data.value) {
    ...
    for (let i = 0; i < rows; i++) recent.value.push(data.value.data[i]);
  }
}
load();
